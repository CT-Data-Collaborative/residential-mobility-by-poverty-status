...
##################################################################
#
# Processing Script for Residential Mobility by Poverty Status
...
# If we're just doing 1 year, we need to change the pattern to reflect that file. 
# This needs a better long term solution, but it is passible for now.
...
for(file in list.files(raw_location, pattern = "ACS_1.*")) {
    ...
    raw[,c("Variable", "Mobility", "Poverty Status") := do.call(Map, c(f = c, str_split(Variable, fixed(";"))))]
    ...
    setkey(estimates, FIPS, Mobility, `Poverty Status`)
    ...
    setkey(moes, FIPS, Mobility, `Poverty Status`)
    ...
    setkey(totals, FIPS, `Poverty Status`)
    setkey(numbers, FIPS, `Poverty Status`)
